...
class ChartRefExpMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        for data_sample in data_samples:
            ...
            target_bbox = data_sample['gt_instances']['bboxes']
            assert len(target_bbox) == 1
            result['target'] = target_bbox[0].cpu().numpy()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        #                  for k in self.topk},
        # }
        score = {k: 0.0 for k in self.topk}
        count = 0.0
        ...
        #         except Exception as e:
        #             print(e)
        results = sorted([v for k, v in score.items()])
        mean_precision = sum(results)
        logger.info(f' Precision @ 1, 5, 10: {results}')
        # results = {}
        # mean_precision = 0.0
        # for key, value in dataset2score.items():
        ...
        out_results = {'mean_precision': mean_precision / 3.0}
